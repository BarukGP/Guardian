...
Las credenciales se cargan exclusivamente desde ``api-service/.env`` (o desde el
...
# Para este piloto local, el archivo api-service/.env es la fuente explícita de la
# configuración. ``override=True`` evita que un proceso recargado conserve una
# variable vacía de una ejecución anterior y oculte el valor recién guardado.
load_dotenv(API_SERVICE_DIR / ".env", override=True)
...
def get_settings() -> Settings:
    ...
    return Settings(
        ...
        guardian_demo_password=os.getenv("GUARDIAN_DEMO_PASSWORD", "").strip(),
    )
...
def get_database_path() -> Path:
    ...
    return Path(configured_path) if configured_path else API_SERVICE_DIR / "data" / "guardian.db"
