...
from app.models.schemas import DepositCreate, DepositWithRisk, TransferCreate, TransferWithRisk
from app.nessie.client import NessieClient, NessieError
from app.nessie.dependencies import get_nessie_client
from app.risk_engine.rules import assess_deposit, assess_transfer
...
def create_transfer(
    ...
    try:
        created: dict[str, Any] = client.create_transfer(account_id, nessie_payload)
        created.setdefault("payee_id", transfer.payee_id)
        created["nessie_persisted"] = True
    except NessieError:
        occurred_fallback = datetime.now(timezone.utc).isoformat()
        created = {
            "_id": f"local-{account_id[:8]}-{int(datetime.now(timezone.utc).timestamp())}",
            "payer_id": account_id,
    ...
    assessment = assess_transfer(
        transfer.amount, transfer.payee_id, priors, occurred_at
    )
